...
def find_random_user():
    group_id = '225394761'  # Замените на ID вашей группы
    members = vk.groups.getMembers(group_id=group_id, fields='sex,city,bdate')['items']
...
for event in longpoll.listen():
    if event.type == VkEventType.MESSAGE_NEW and event.to_me:
        if event.text.lower() == "привет":
            ...
            random_user = find_random_user()
            # Получаем информацию о случайном пользователе и отправляем ее
            random_user_info = vk.users.get(user_ids=random_user['id'], fields='photo_max_orig,bdate,city,sex')[0]
            download_photo_and_save(random_user['id'], save_path)
            send_photo_and_info(event.user_id, save_path, random_user_info)
